...
def _make_storage_pool(tmp, device_id, source_id="loop-001"):
...
class TestTakeover:
    ...
    def test_local_owned_write_keeps_position(self):
        ...
        storage_pool, store, _src = _make_storage_pool(d, "dev-A")
        try:
            storage_pool.write_file("/local.txt", b"v1")
    ...
    def test_legacy_null_owner_write_overwrites(self):
        ...
        try:
            ...
            storage_pool.sources[0].write("legacy_phys.txt", b"old")
...
class TestOrphanGC:
    ...
    def test_gc_removes_unreferenced_file(self):
        ...
        d = tempfile.mkdtemp()
        storage_pool, store, src = _make_storage_pool(d, "dev-A")
        try:
            # 활성 레코드 1개 (보존 대상)
            storage_pool.write_file("/keep.txt", b"keep")
            ...
            removed = storage_pool.gc_orphan_files()

            assert removed == 1
            assert not src.exists(orphan)
            # 보존 대상은 그대로
            assert src.exists(keep_rec.physical_path)
            assert storage_pool.read_file("/keep.txt") == b"keep"
        finally:
            store.close()
            shutil.rmtree(d, ignore_errors=True)
    ...
    def test_gc_skips_when_device_id_none(self):
        ...
        d = tempfile.mkdtemp()
        storage_pool, store, src = _make_storage_pool(d, None)
        try:
            some = "2" * 32 + "_data.txt"
    ...
    def test_gc_if_needed_debounce(self):
        ...
        try:
            ...
            assert storage_pool.gc_orphan_files_if_needed() == 0
            assert src.exists(orphan1)

            # 플래그 set → 1회 동작
            storage_pool.mark_gc_needed()
            removed = storage_pool.gc_orphan_files_if_needed()
            assert removed == 1
    ...
    def test_takeover_then_gc_full_cycle(self):
        ...
        try:
            ...
            storage_pool.write_file("/shared.txt", b"new data")
            ...
            assert storage_pool.read_file("/shared.txt") == b"new data"
        finally:
            store.close()
            shutil.rmtree(d, ignore_errors=True)
