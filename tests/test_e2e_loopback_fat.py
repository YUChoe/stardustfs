...
config → _build_core(암호화+메타+스토리지 풀 조립) → write_file(암호화 후 FAT 이미지 내부
저장) → read_file(복호화) → 재마운트 후 영속성 → FAT 소스 간 스토리지 풀 스필오버 → 삭제.
...
def _build(cfg):
    ...
    storage_pool, meta, enc, _db = _build_core(cfg)
    return storage_pool, meta


def test_e2e_write_read_persist_spillover(tmp_path):
    cfg = _config(tmp_path)
    storage_pool, meta = _build(cfg)

    # 1) 쓰기/읽기 라운드트립 — 파일이 FAT 이미지 내부에 저장된다.
    data = os.urandom(300 * 1024)
    storage_pool.write_file("/docs/report.bin", data)
    assert storage_pool.read_file("/docs/report.bin") == data
    ...
    src = storage_pool._get_source_by_id(m.source_id)
    cipher = src.read(m.physical_path)
    assert cipher != data and len(cipher) >= len(data)

    # 2) FAT 소스 간 스토리지 풀 스필오버: loop-a를 채운 뒤 큰 파일은 loop-b로.
    storage_pool.write_file("/big_a.bin", os.urandom(6 * 1024 * 1024))  # loop-a 채움
    storage_pool.write_file("/big_b.bin", os.urandom(5 * 1024 * 1024))  # a 부족 → b
    ma = meta.lookup("/big_a.bin")
    mb = meta.lookup("/big_b.bin")
    assert ma.source_id != mb.source_id  # 서로 다른 이미지에 분산
    assert storage_pool.read_file("/big_b.bin") is not None

    # 3) 재마운트 영속성: 핸들 닫고 다시 조립해도 파일이 그대로 읽힌다.
    storage_pool.close_local_sources()
    meta.close()
    storage_pool2, meta2 = _build(cfg)
    assert storage_pool2.read_file("/docs/report.bin") == data

    # 4) 삭제 후 가용 공간 회수.
    m2 = meta2.lookup("/big_a.bin")
    src_a = storage_pool2._get_source_by_id(m2.source_id)
    before = src_a.get_available_space()
    storage_pool2.delete_file("/big_a.bin")
    after = src_a.get_available_space()
    assert after >= before

    storage_pool2.close_local_sources()
    meta2.close()
...
def test_e2e_capacity_exhausted_raises(tmp_path):
    ...
    storage_pool, meta = _build(cfg)
    try:
        storage_pool.write_file("/f1.bin", os.urandom(8 * 1024 * 1024))  # loop-a
        storage_pool.write_file("/f2.bin", os.urandom(8 * 1024 * 1024))  # loop-b
        with pytest.raises(InsufficientStorageError):
            storage_pool.write_file("/f3.bin", os.urandom(8 * 1024 * 1024))  # 둘 다 부족
    finally:
        storage_pool.close_local_sources()
        meta.close()
