...
def test_evacuate_blocks_when_no_capacity(tmp_path):
    ...
    storage_pool, store = _storage_pool(tmp_path, [a])
    phys = storage_pool._generate_physical_path("/f")
    a.write(phys, b"data")
    store.insert("/f", "only", phys, 4, 0.0, 0.0, device_id="devA")

    report = storage_pool.evacuate_source("only")
...
def test_build_local_source_inventory(tmp_path):
    ...
    inv = {i["source_id"]: i for i in build_local_source_inventory(storage_pool)}
...
def test_inventory_excludes_remote(tmp_path):
    ...
    storage_pool, store = _storage_pool(tmp_path, [a])
    storage_pool.register_remote_device("dev-b", _FakeRemote())
    # 원격 디바이스는 storage_pool.sources에 없으면 자연히 제외되지만, 마운트된 원격
    # 소스가 sources에 있어도 is_remote로 걸러짐을 가정. 여기서는 로컬만 신고됨.
    inv = build_local_source_inventory(storage_pool)
...
def test_replace_local_sources_keeps_remote_and_recover(tmp_path):
    ...
    sentinel = object()
    storage_pool._recover_fn = sentinel

    storage_pool.replace_local_sources([b])  # old-a → old-b 교체
    local_ids = [s.source_id for s in storage_pool.sources if not getattr(s, "is_remote", False)]
    assert local_ids == ["old-b"]            # 로컬 교체됨
    assert storage_pool._get_source_by_id("old-b") is b
    assert storage_pool._get_source_by_id("old-a") is None
    assert storage_pool._remote_devices.get("dev-b") is not None  # 원격 보존
    assert storage_pool._recover_fn is sentinel                   # 복구 콜백 보존
    store.close()
...
def test_write_to_remote_refreshes_inactive_remote(tmp_path):
    ...
    storage_pool.register_remote_device("dev-b", rem)
...
def test_write_raises_when_local_full_and_no_reachable_remote(tmp_path):
    ...
    storage_pool, store = _storage_pool(tmp_path, [])
    storage_pool.register_remote_device("dev-b", _FakeRemote(active=False))  # 오프라인
    with pytest.raises(InsufficientStorageError):
        storage_pool.write_file("/x", b"data")
    store.close()
...
def test_write_prefers_local_when_space_available(tmp_path):
    ...
    remote = _FakeRemote()
    storage_pool.register_remote_device("dev-b", remote)
    storage_pool.write_file("/s", b"small")
    meta = store.lookup("/s")
...
def test_evict_cold_deletes_local_only_when_safe(tmp_path):
    ...
    safe = {"/safe"}
    report = storage_pool.evict_cold(lambda vp: vp in safe, bytes_to_free=10**9)
...
def test_read_evicted_triggers_recover(tmp_path):
    ...
    def fake_recover(vp):
        ...
        storage_pool.write_file(vp, b"data")

    storage_pool._recover_fn = fake_recover
    assert storage_pool.read_file("/f") == b"data"
...
def test_read_evicted_without_recover_raises(tmp_path):
    ...
    with pytest.raises(OSError):
        storage_pool.read_file("/f")  # 복구 콜백 없음 → 온라인 필요 에러
    store.close()
...
def test_evacuate_empty_source_ok(tmp_path):
    ...
    storage_pool, store = _storage_pool(tmp_path, [a, b])
    report = storage_pool.evacuate_source("e-a")
...
def test_evacuate_unmoved_when_remote_offline(tmp_path):
    ...
    storage_pool, store = _storage_pool(tmp_path, [a])
    storage_pool.register_remote_device("dev-b", _FakeRemote(active=False))
    phys = storage_pool._generate_physical_path("/f")
    a.write(phys, b"x")
    store.insert("/f", "only2", phys, 1, 0.0, 0.0, device_id="devA")

    report = storage_pool.evacuate_source("only2")
...
def test_detach_source_removes_from_config_when_evacuated(tmp_path, monkeypatch):
    # detach_source는 캐시된 오프라인 세션을 쓰므로, 세션의 storage_pool.evacuate_source를
