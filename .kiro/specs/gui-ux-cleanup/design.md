...
툴바 버튼(동작 설명 + 비활성 시 활성 조건), daemon 라벨(pid), 관리 트리 소스 행
(전체 경로·소스 ID)에 붙인다. 표시 위치는 커서 기준이다 — 위젯 기준으로 두면
Treeview처럼 큰 위젯에서 툴팁이 위젯 아래 다른 영역 위에 뜬다.

파일 목록 셀에는 붙이지 않는다. Treeview는 위젯 단위라 `<Motion>`으로 대상 셀을
판별해 문구를 바꿔도 툴팁 자체는 목록 밖에 떠, 어느 셀 설명인지 읽히지 않는다.
