...
- backup: 저장된 at-rest 청크를 재분할·재암호화 없이 그대로 복제 청크로 쓴다(저장 단위가
  ...
  모든 청크가 목표 복제본 수(`min_replicas`, 기본 1=원본 외 1부)를 확보하면 replicated,
  아니면 pending(경고). file_ref/chunk_id는 가상경로 SHA-256(서버에 경로 비노출).
  placement에는 목표보다 `PLACEMENT_SPARE`(2)개 많은 후보를 요청해 일부 홀더가
  실패해도 같은 청크에서 대체 홀더로 넘어가고, 목표를 채우면 남은 후보는 쓰지 않는다.
- 배치 제외: 자기 device, 그 청크의 **원본을 보관한 device**, 보관 한도 초과 device를
  후보에서 뺀다. 원본이 있는 기기에 사본을 두면 내구성 이득이 없기 때문이고, 배제
  기준이 "실행 중인 기기"가 아니라 원본 위치인 이유도 같다(소유는 사용자 단위). 제외 후
  후보가 하나도 없으면 조용히 성공 처리하지 않고 pending + 사유를 경고로 남긴다.
- 수동 백업 위임: GUI/CLI에서 백업을 요청한 파일의 청크가 다른 기기에 있으면, 그 데이터를
  ...
  로컬 전송을 강행하지 않는다.
- 재개: pending 재시도는 전체 파일을 다시 올리지 않는다. 서버에 등록된 청크 해시가
  현재 at-rest 청크와 같고 online 복제본이 이미 목표를 채웠으면 그 청크의 push를
  생략한다(조회 1회로 4 MiB 전송을 대체). 해시가 다르면(파일 수정) 낡은 사본이므로
  다시 올리고, 홀더가 오프라인이 되어 목표에 미달하면 새 홀더로 올린다.
