...
| 정책 프로비저닝 | P2P·호스팅 허용과 호스팅 상한을 서버 정책으로 (기본 활성) | 완료 |
| 청크 3카피 | 카피 수와 기기 수를 따로 집계, 다른 기기 우선 배치, heal 이전 | 완료 |
| 복제 가시성 | 사용자 단위 소유 모델 정정 + 백업 진행 표시(`/ctl/progress`) | 완료 |
| GUI UX 정리 | 화면 영역별 분해, 선택 기반 액션, 폴링 축소, 단일 인스턴스 | 완료 (mvp16) |
